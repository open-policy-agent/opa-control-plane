...
// A PrefixFS is a simple in-memory file system for creating new
// [fs.FS] from existing ones, under specific prefixes.
//
...
// var _ fs.ReadLinkFS = PrefixFS(nil)
...
func (d *mntDir) ReadDir(_ int) ([]fs.DirEntry, error) {
	// return d.fsys.(fs.ReadDirFS).ReadDir(".") // NB: ignores count
	return fs.ReadDir(d.fsys, ".")
}
