	...
	for databaseType, databaseConfig := range dbs.Configs(t) {
		t.Run(databaseType, func(t *testing.T) {
			...
			tests := []struct {
				...
				{
					name:   "PUT deeply nested for browsing",
					method: "PUT",
					path:   "/v1/sources/system1/data/a/bc/def",
					body:   `{}`,
					result: map[string]any{},
				},
				{
					name:   "PUT deeply nested for browsing (2)",
					method: "PUT",
					path:   "/v1/sources/system1/data/a/bc",
					body:   `{"xy": true}`,
					result: map[string]any{},
				},
				{
					name:   "PUT deeply nested for browsing (3)",
					method: "PUT",
					path:   "/v1/sources/system1/data/a/aaa",
					body:   `{}`,
					result: map[string]any{},
				},
				{
					name:   "GET up the tree",
					method: "GET",
					path:   "/v1/sources/system1/data/a/bc",
					body:   "",
					result: map[string]any{
						"result": map[string]any{"xy": true},
						"paths":  []any{"def"},
					},
				},
				{
					name:   "GET up the tree (2)",
					method: "GET",
					path:   "/v1/sources/system1/data/a",
					body:   "",
					result: map[string]any{
						"paths": []any{"aaa", "bc"},
					},
			...
			for _, test := range tests {
				t.Run(test.name, func(t *testing.T) {
					status := std_cmp.Or(test.statusCode, 200)
					apikey := std_cmp.Or(test.apikey, adminKey)
					tr := ts.Request(test.method, test.path, test.body, apikey).ExpectStatus(status)
